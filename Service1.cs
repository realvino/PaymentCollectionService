...
namespace WindowsServiceCS
{
    ...
    private void SchedularCallback(object sender, System.Timers.ElapsedEventArgs e)
    {
        ...
        try
        {
            var ftpdirectory = new DirectoryInfo(@"C:\ftp\data\TallyReceiptExport\");
                ...
                foreach (var filename in myFile)
                {
                    ...
                    if (DbFileName == "")
                    {
                        ...
                        using (WebClient ftpClient = new WebClient())
                        {
                            ...
                            {
                                using (SqlCommand spcmd = new SqlCommand("sp_ReadFile_PaymentCollection", connnn))
                                {
                                    spcmd.CommandType = CommandType.StoredProcedure;

                                    spcmd.Parameters.Add("@Path", SqlDbType.VarChar).Value = @"C:\ftp\data\TallyReceiptExport\" + filename.Name;
        ...
        {
            string path = "C:\\hostinglog\\PaymentCollectionTestServiceLog.txt";
